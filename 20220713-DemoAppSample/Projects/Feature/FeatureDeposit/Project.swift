...
let targets: [Target] = [
    ...
    .init(name: "FeatureDepositUnitTests",
          platform: .iOS,
          product: .unitTests,
          bundleId: "kr.minsone.feature.deposit.unitTests",
          deploymentTarget: .iOS(targetVersion: "13.0", devices: [.iphone, .ipad]),
          sources: ["Tests/UIUnitTests/**"],
          dependencies: [.target(name: "FeatureDeposit"),
                         .target(name: "FeatureDepositDemoApp")],
          settings: .settings(base: ["TEST_HOST": "",
                                     "BUNDLE_LOADER": "$(BUILT_PRODUCTS_DIR)/$(TEST_TARGET_NAME).app/$(TEST_TARGET_NAME)"])
         ),
]
...
let project: Project =
    .init(name: "FeatureDeposit",
          ...
          targets: targets,
          schemes: [.init(name: "FeatureDepositUI",
                          shared: true,
                          testAction: .targets(["FeatureDepositUIUnitTests"])),
                    .init(name: "FeatureDeposit",
                          shared: true,
                          testAction: .targets(["FeatureDepositUnitTests"])),
          ])
